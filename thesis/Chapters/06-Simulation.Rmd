...
Finally, the chapter presents the results of applying both the state-of-the-art outbreak detection algorithms and the novel outbreak detection algorithm to the simulated data. The performance of each algorithm is evaluated based on the simulation results.
...
One of the primary measures used is the False Positive Rate (FPR), which is calculated for each of the 28 scenarios and methods. The FPR is determined by calculating the proportion of the 49 weeks and 100 replicates in which the observed value exceeds the threshold, even though there are no actual outbreaks during those weeks.
...
In this section, the results regarding the simulation study is presented. Both the state-of-the-art outbreak detection algorithm and the novel outbreak detection algorithm are applied to the simulated data. 

The `control` arguements for the Farrington and Noufaily method respectively can be found in Appendix \@ref(controlsStateOfTheArt).


For both of the state-of-the-art outbreak detection algorithms 


In all of the methods, a 


The model for the fixed effects in each of the modeling frameworks respectively, is defined as:
...
in each of the modeling frameworks
