...
First, the method for simulating the baseline data is presented. These data are generated using a negative binomial model with a time-dependent mean $\mu(t)$. Hereafter, the assumptions regarding the simulated outbreaks are outlined, i.e. outbreak size and distribution in time. Then, the evaluation measures are presented just before the results.
...
o evaluate the performance of the outbreak detection system, several measures are employed to assess its effectiveness in detecting current outbreaks. These measures are specifically designed to capture relevant quantities in the given context.
...
As expected, the PODs increase with $k$
