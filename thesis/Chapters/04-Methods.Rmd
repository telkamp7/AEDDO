...
Then, in the second step, as a new observation becomes available, the algorithm infers the one-step ahead random effect $u_{i{t}_1}$ for each group using the obtained model estimates $\hat{\theta}_{t_0}$. The threshold $U_{t_0}$ for detecting outbreaks is defined as a quantile of the distribution of the random effects in the second stage model. This can be either a Gaussian distribution using the plug-in estimate $\hat{\sigma}$ or a Gamma distribution using the plug-in estimate $\hat{\phi}$.
