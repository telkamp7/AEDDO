...
It can be shown, that the combined models for the fixed effects minimizes the logarithmic score for both modeling frameworks, indicating a better fit to the \gls{STEC} data. As a result, these models have been selected for further investigation regarding their ability to detect outbreaks.
...
Notably, a significant drop in the trend is observed in the mid-2020, immediately after the \gls{COVID-19} lockdown, lasting until the mid-2021. Since then, the trend has been steadily increasing and is currently at an all-time high.
...
To further understand the variation in the seasonal component, the amplitude ($\omega$) and phase ($\rho$) are calculated using the formulas:

\begin{align}
\omega = \sqrt{(\beta_{\sin}^2 + \beta_{\cos}^2)} & \quad \rho = -\arctan \bigg(\frac{\beta_{\cos}}{\beta_{\sin}}\bigg)
\end{align}

Due to the challenges of propagating uncertainty estimates, bootstrap confidence intervals are calculated based on 10,000 replicates for each parameter. The maximum likelihood estimate for the amplitude is visualized along with its bootstrap confidence interval in Figure \@ref(fig:AmplitudePhase).

(ref:AmplitudePhase) Illustration of the amplitude \textbf{(a)} and phase \textbf{(b)} along with a $95\%$ bootstrap confidence interval for each of the modeling frameworks.
