...
Convergence issues were observed in the hierarchical models during the case studies, particularly in the \textit{Listeriosis} (LIST) case study. Only the constant fixed effects models were able to converge, indicating challenges with the other models. The low incidence rate in the LIST data and the absence of overdispersion are likely factors contributing to these convergence difficulties.
...
Effectively handling previous outbreaks in statistical outbreak detection requires domain-specific knowledge from epidemiologists to determine the most appropriate approach. While the novel method completely excludes past outbreaks, this may not be the optimal practice as it reduces the sample window. A clear example of this can be seen in the \textit{Salmonellosis} (SALM) case study. In Figure \@ref(fig:handlealarms), the effective number of observations used for parameter estimation at a given point in time is visualized.

(ref:handlealarms) Effective number of observations in the SALM case study, that enter the parameter estimation at a given point in time.
...
Figure \@ref(fig:CompareAlarmsLIST) demonstrates that the state-of-the-art outbreak detection algorithms, especially the Farrington method, raise a higher number of alarms compared to the novel algorithms in the \textit{Listeriosis} (LIST) case study.
...
In the \textit{Shigellosis} (SHIL) case study, the incidence of cases is slightly higher, resulting in a relatively higher number of alarms generated by both the state-of-the-art and novel outbreak detection methods. This can be observed in Figure \@ref(fig:CompareAlarmsSHIL). It is important to note that only one well-documented outbreak of SHIL was investigated by SSI and collaborating organizations. As a result, the number of alarms generated by the methods may be overestimated in terms of the priority given to investigating outbreaks in this specific disease.
...
When examining the Shiga toxin (verotoxin)-producing \textit{Escherichia coli} (STEC) data, the substantial difference in sensitivity between the state-of-the-art and novel outbreak detection methods becomes evident. Figure \@ref(fig:CompareAlarmsSTEC) clearly demonstrates that both state-of-the-art methods generate a significant number of alarms. This excessive number of alarms raises concerns about the practicality and feasibility of these methods in real-world applications.

(ref:CompareAlarmsSTEC) Alarm plot displaying alarms for the STEC time series using four different algorithms, along with outbreaks investigated by SSI.
